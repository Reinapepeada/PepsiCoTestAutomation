...
class TestTC01Recipes():
  ...
  def test_tC01Recipes(self):
    ...
    self.driver.find_element(By.XPATH,'/html/body/div[1]/div[3]/div/div[2]/div/div/div[2]/div/div[1]/div/div[2]/div/div/div/div/div/ul/li[2]/table/tbody/tr/td/div/a').click()
    time.sleep(2)
    # ----------------------
    # 11 click en Recetas incompletas li
    self.driver.find_element(By.XPATH,'/html/body/ul[2]/li[7]/table/tbody/tr/td/div/a/span').click()
    time.sleep(6)
    #reporte de recetas = [] con enunciados
    reporteRecetas=[['SKU','Description','Name Plate','Bag Nominal W','Seasoning Level','Bags Case','Sleeves Case','Sleeves Bags','Fryer Asignment','UOM','Oil Level']]
    ...
    for i in range(1,20):
      try:
        ...
        self.driver.execute_script(f'return {itemPath}({i}) > ptcs-div")').click()
        time.sleep(5)
        # 16 analizo la tabla
        ...
        if fila>1:
          for f in range(2,fila+1):
            ...
            sku=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[1]').text
            description=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[2]').text
            namePlate=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[3]').text
            bagNominalW=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[4]').text
            seasoningLevel=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[5]').text
            bagsCase=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[6]').text
            sleevesCase=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[7]').text
            sleevesBags=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[8]').text
            fryerAsignment=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[9]').text
            uom=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[10]').text
            oilLevel=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[11]').text

            # 19 agrego los datos a la lista
            reporteRecetas.append([sku,description,namePlate,bagNominalW,seasoningLevel,bagsCase,sleevesCase,sleevesBags,fryerAsignment,uom,oilLevel])  
      except:
        break
      
    # 20 hago click en administrador drop down que esta en el nav 
    self.driver.find_element(By.XPATH,'/html/body/div[1]/div[3]/div/div[2]/div/div/div[2]/div/div[1]/div/div[2]/div/div/div/div/div/ul/li[3]/table/tbody/tr/td/div/a/span').click()
    time.sleep(2)
    ...
    if fila>1:
      for f in range(2,fila+1):
        ...
        productoID=self.driver.find_element(By.XPATH,f'{tablePath}/tr[{f}]/td[1]').text
    ...
    path2 = pathOG + "/tables/totalProductos_"+PLANTA+".xlsx"    # creo el path reporte de excel con fecha y hora
    variable = time.strftime("%Y-%m-%d_%H-%M-%S") 

    pathExcel = pathOG + "/reportOutput/ReporteIncompletasRecetas"+str(variable)+".xlsx"
