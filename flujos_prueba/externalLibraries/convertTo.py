...
def creartablaExcel(report,path,nombreHoja):
    # creo un libro de trabajo
    wb = Workbook()
    # creo una hoja de trabajo
    ws = wb.active
    letra=chr(ord('a')+len(report[0])-1)

    # creo una tabla con los datos de la lista
    table = Table(displayName=nombreHoja, ref=f"A1:{letra}" + str(len(report)))
    ...
    # agrando las celdas
    for i in range(1,len(report[0])+1):
