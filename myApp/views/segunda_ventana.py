...
class SegundaVentana:
    def __init__(self, ventana_principal, ubicacion):
        ...
        self.etiqueta.pack()
        self.etiqueta = tk.Label(self.ventana, text="Selecciona una un flujo de prueba para ejecutarlo")
        self.etiqueta.pack(pady=10)
        
        for flujo in range(len(flujos)-1):
            boton_ruta = tk.Button(self.ventana, text=flujos[flujo][0], command=lambda flujo=flujos[flujo]: ejecutar_comando_con_ruta(f"C:/Users/09384038/Desktop/proyect/PepsiCoTestAutomation/flujos_prueba/{self.ubicacion}/{flujo[1]}", 'pytest'))
            boton_ruta.pack()
