...
class TestUC05_M_TC03_PC1_DP_Wastetab():
  ...
  def test_tUC04_M_TC05_PC1_DP_Wastetab(self):
    ...
    for f in range(2,filaslen+1) :
      ...
      #obteniendo el valor de over y under
      over=self.driver.find_element(By.XPATH,pathTabla+'/tr['+str(f)+']/td['+str(6)+']').text
      under=self.driver.find_element(By.XPATH,pathTabla+'/tr['+str(f)+']/td['+str(7)+']').text
      print(f"tubo {tuboNombre} totalBags {totalBags} over {over} under {under}")
      # Validando que si totalBags esta en mas de 1000 entonces over y under no deben ser 0
      if float(totalBags)>1000 and (float(over)==0 or float(under)==0): 
